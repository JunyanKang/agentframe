...
## Handoff Rules
- Handoff to specification for any task that changes behavior, APIs, data, configuration, compatibility, or release process.
- Handoff to architect when task breakdown exposes unresolved design decisions.
- Handoff to implementer only for tasks with clear acceptance criteria and no missing specification.
- Handoff to project_memory when roadmap, priorities, or open questions change.
...
- Coordinate with architect when design decisions drive task boundaries.
- Coordinate with tester to make validation gates concrete.
