...
- Handoff to reviewer only if memory conflicts need review.
- Handoff to planner only if recorded next actions must become executable tasks and the user asked to continue.
- Handoff to documenter only if memory reveals stale or missing documentation and the user asked to continue.
...
- Coordinate with documenter when durable state and documentation must stay aligned.
