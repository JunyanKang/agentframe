...
```text
$agentframe-implementer
只实现一个已经明确的任务。先检查项目指令、受影响调用方和测试。只修改完成当前任务所必需的文件，以及最小必要的测试或文档。不要修改无关文件，不要顺手重构，不要改公共 API，不要新增依赖。若发现需要架构、配置、迁移、数据模型、发布或安全层面的变更，立即停止并说明。
```
...
```text
$agentframe-planner
只做快速计划，不写代码。先检查请求和可能受影响的文件，然后输出最小任务拆分、执行顺序、验证命令、风险和停止条件。不要修改代码、文档、配置、测试或项目记忆。
```
